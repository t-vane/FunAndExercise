...
def tune(train_fn, param_vals, train_feats, train_labels, val_feats, val_labels):
    ...
    for i, val in enumerate(param_vals):
        ...
        train_preds = fs.classify(train_feats, theta, theta_0)
        train_accs[i] = fs.accuracy(train_preds, train_labels)

        val_preds = fs.classify(val_feats, theta, theta_0)
        val_accs[i] = fs.accuracy(val_preds, val_labels)

    return train_accs, val_accs

def tune_perceptron(*args):
    return tune(fs.perceptron, *args)

def tune_avg_perceptron(*args):
    return tune(fs.average_perceptron, *args)

def tune_pegasos_T(best_L, *args):
    def train_fn(features, labels, T):
        return fs.pegasos(features, labels, T, best_L)
    return tune(train_fn, *args)

def tune_pegasos_L(best_T, *args):
    def train_fn(features, labels, L):
        return fs.pegasos(features, labels, best_T, L)
    return tune(train_fn, *args)
